...
#
# Run as root: sudo bash userfortify-no-firewall.sh
# -----------------------------------------------------------------------------

set -euo pipefail

# Fortify - Harden Linux servers safely (interactive)
# Supports: Ubuntu/Debian + RHEL/Rocky/Alma
# Author: tdiprima
...
title() {
  echo
  echo -e "${COLOR_BOLD}Safe Linux Server Hardening${COLOR_RESET}"
  echo "------------------------------------------------"
}
...
main() {
  ...
  HARDEN_USER="$(ask_input "Admin username to ensure exists" "user")"
